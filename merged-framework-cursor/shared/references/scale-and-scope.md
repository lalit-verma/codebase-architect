...
- expect some subsystems to trigger recursive decomposition
...
- recursive decomposition is likely needed for any subsystem that
  gets a deep dive
...
- sample repetitive leaf handlers, adapters, or tests after a pattern
  is established
...
A subsystem qualifies for sub-module decomposition when **any** of
these hold:

- the subsystem directory contains **>60 source files**
- the subsystem has **>3 clearly distinct internal responsibilities**
  that could each stand alone
- during the deep dive, the agent cannot produce a quality document
  without splitting because the subsystem is too internally diverse

### How Recursive Decomposition Works

1. **During Phase 1 (Discover):** Flag candidate subsystems in the
   checkpoint with a note like "Candidate for sub-module split: >80
   files, appears to contain auth, routing, and middleware as distinct
   concerns."
2. **During Phase 2 (Deep Dive):** At the start of the deep dive for
   a flagged subsystem (or if the agent discovers it during reading):
   - Propose sub-modules to the user with names, paths, and
     responsibilities.
   - Get confirmation before splitting.
   - Write a **parent subsystem doc** (overview, how sub-modules
     relate, shared patterns).
   - Run a sub-module deep dive for each child.
3. **Depth limit:** Maximum 3 levels total (system → subsystem →
   sub-module). Do not recurse further.
4. **State tracking:** Track sub-module progress in
   `.analysis-state.md` alongside subsystem progress.

### Sub-Module Document Weight

Sub-module docs should be lighter than subsystem docs:

- drop: standalone Design Decisions section (captured at parent level)
- drop: standalone Configuration section (usually inherited from parent)
- keep: Boundaries, Contracts, Flows, Dependencies, Patterns,
  Modification Guide, Edge Cases, Open Questions
